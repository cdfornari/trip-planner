...
import {
  PaymentFailed,
  PaymentFailedEvent,
} from 'libs/trip-plans/domain/events/payment-failed.event';
import {
  VehicleRentalBooked,
...
@SagaStep
@Injectable()
export class ActivityBookingListener {
  ...
  @SubscribeToGroup(PaymentFailed.name, SUBSCRIPTION_GROUP_COMPENSATION)
  async compensate(
